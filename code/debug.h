...
#define dbgtrace(MODULE, ...) if constexpr (DBG_##MODULE) dbg::core_trace(__VA_ARGS__) ;

namespace dbg {
  extern void core_trace(const char* text, usize value);
  ...
  extern void printhexln(const char* text, const usize var, const char symbol);
